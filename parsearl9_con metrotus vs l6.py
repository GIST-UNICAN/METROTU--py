...
def itera_vehiculo(vehiculo):
    for row in ws1:
        if row[2].value==vehiculo:
            yield row
filtra_excel.filtra_excel(xlsx_entrada,9,(339,511),(tuple(range(1,29))))   
wb = load_workbook(filename = xlsx)
ws1 = wb.active
ws3 = wb.create_sheet(title="tv")
ws3.append(('coche','hora cabecera','repuente sardinero','ahora no va al centro','null', 'duracion trayecto'))
...
for vehiculo in conjunto_vehiculos:
    ...
    for row in itera_vehiculo(vehiculo):
        if row[3].value==511 and fila_anterior[3].value==339:
...
wb.save(filename = ruta+'/'+nombre+"_parseada_mtus_vs6.xlsx")    
