...
class bar():
    ...
    value = None
# open the file
query = ("SELECT Instante FROM `pasos_parada` where linea=100 and parada=512 ORDER BY `Instante` asc")
...
ruta=os.getcwd()
nombre='COORDINACION_VALDECILLA_BARRIOS'
xlsx = (ruta+"/"+nombre+'.xlsx')
xlsx_entrada =(ruta+"/"+'Libro1.xlsx')
filtra_excel.filtra_excel_coordinacion(nombre,xlsx_entrada,(17,3,13,),(512,),(20,))        
...
def check_cnx(fecha):
    ...
    if espera > timedelta(hours=1):
        ...
        return (0, espera,llegada_central) if espera > timedelta(minutes=5) else (1, espera,llegada_central)
...
for linea in conjunto_linea:
    ...
    for row in itera_linea(linea):
        if fila_anterior[5].value: #row[1].value==3:
            ...
            if (dt1-dt2 < timedelta(minutes=3) or linea is 14) and conexion:
                if conexion[0] == 0:
