...
def slices(
    s,
        args=(0, 31, 37, 45, 51, 58, 67, 75, 87, 92, 98,
              129, 153, 177, 200, 201, 203, 205, -1),
        ...
        for raw_row in iter_archivo:
            try:
                ...
                if row[6] == 'NULL':
                    ...
                    row[6] = datetime.strptime(
                        str(row[6]), "%Y-%m-%d %H:%M:%S.%f")  # hlleg
                if row[7] == 'NULL':
                    row[7] = 'Null'
                else:
                    row[7] = datetime.strptime(
                        str(row[7]), "%Y-%m-%d %H:%M:%S.%f")  # hsal
                # insertamos en la bd
...
#            print('salidas')
    ...
    querie = "INSERT INTO `pasos_parada_ajustada` (`Linea`, `Coche`, `Viaje`, `Parada`, `Instante`, `Nombre`, `PSuben`, `Fecha`, `tipo_lectura`, `Sublinea`) VALUES " + str(
...
filtra_excel(
    r"D:\Users\Andres\OneDrive\OneDrive - Universidad de Cantabria\Recordar GIST - VARIOS\datos_tus\2022vh.rpt"
)
