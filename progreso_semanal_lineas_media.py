...
#            df['dia_real'] = pd.DatetimeIndex(df['salida']).day
            #normalizamos todos los dias como reales para la media
            df['dia'] = 1#'media'
...
for sentido, dataframe in lista_df.items():
    ...
    for key in grupo_linea.groups.keys():
        df=grupo_linea.get_group(key)
        df=df.groupby(pd.cut(df["hora salida"], np.arange(0, 24, 0.3), 'all')).mean()
        df.dropna(inplace=True)
        diccionario_linea_sentido[sentido][key[0]].append(
            df)

# se genera el directorio donde van los archivos
directorio = 'variacion_semanal_media_{}_{}'.format(dia_primero, dia_hasta)
...
#                print("SENTIDO: "+sentido+" LINEA: "+str(linea)+" grupos: "+str(list(horas_corte)))
                ...
                for x, y in zip(x_s, y_s):
                    if str(df_dia["dia"].iloc[0]) !='0.0':
                        pass
#                        plt.scatter(x, y, color=color)
                    label= 'teorico' if str(df_dia["dia"].iloc[0]) =='0.0' else 'media'
                    handle, = plt.plot(x, y, color=color, linestyle=estilo, label=label)
...
#                    plt.scatter(x, y, color=color)
                ...
                handles.append(handle)
                label= 'teorico' if str(dia.tolist()[0]) =='0.0' else 'media'
                labels.append(label)
...
with open(directorio+'\\'+'informe.html', 'w') as file:
    ...
    create_pdf(
        ...
        directorio+'\\'+'resumen_semanal_media_{}-{}-{}_al_{}-{}-{}.pdf'.format(dia_inicio.year, dia_inicio.month, dia_inicio.day,
