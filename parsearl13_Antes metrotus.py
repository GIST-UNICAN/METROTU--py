...
def itera_vehiculo(vehiculo):
    for row in ws1:
        if row[2].value==vehiculo:
            yield row
filtra_excel.filtra_excel(xlsx_entrada,13,(252,297,326,250),(tuple(range(1,29))))   
wb = load_workbook(filename = xlsx)
ws1 = wb.active
ws3 = wb.create_sheet(title="tv")
ws3.append(('coche','hora cabecera','C. Polio Autonomía 8','J. Trevilla 16 Arriba 31','null', 'duracion trayecto'))
...
for vehiculo in conjunto_vehiculos:
    ...
    for row in itera_vehiculo(vehiculo):
        if row[3].value==297 and fila_anterior[3].value==252:
        ...
        if row[3].value==250 and fila_anterior[3].value==326:
            t1=row[5].value
            t2=fila_anterior[5].value
            dt1 = timedelta(hours=t1.hour, minutes=t1.minute, seconds=t1.second)
            dt2 = timedelta(hours=t2.hour, minutes=t2.minute, seconds=t2.second)
