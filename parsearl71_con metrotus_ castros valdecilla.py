...
def itera_vehiculo(vehiculo):
    for row in ws1:
        if row[2].value==vehiculo:
            yield row
filtra_excel.filtra_excel(xlsx_entrada,71,(75,512),tuple(range(1,16)))    
wb = load_workbook(filename = xlsx)
ws1 = wb.active
ws3 = wb.create_sheet(title="tv")
ws3.append(('coche','hora cabecera','Los Castros Valdecilla','Coger de la 7c2','None', 'duracion trayecto'))
...
for vehiculo in conjunto_vehiculos:
    ...
    for row in itera_vehiculo(vehiculo):
        if row[3].value==512 and fila_anterior[3].value==75:
...
wb.save(filename = ruta+'/'+nombre+"_parseada_mtus_Castros valdecilla.xlsx")    
